*! version 4.1.6  December 17, 2003 @ 15:18:22
program define reshape7
	/* 4.1.6 - bugfix which keeps variable label for the constant and j variables, too */
...
*	set trace on
	...
	if "`1'"=="clear" {
		...
		char _dta[ReS_Xij]
		char _dta[Res_Xi]
		char _dta[ReS_Xi_lab]
		char _dta[ReS_atwl]
		char _dta[ReS_str]
		char _dta[ReS_Xij_lab]
		exit
	}
...
program define Simple /* {wide|long} <funnylist>, i(varlist)
	...
	reshape7 clear
	reshape7 i `i'
	reshape7 j `jvar' `jvals' `string'
	reshape7 xij `list' `atwl'
end

program define Xi /* simple variable names */
	if "`*'"!="" {
		unab xi_names : `*'
		foreach var of local xi_names {
			local theLab : var lab `var'
			local lablist "`lablist' `"`theLab'"'"
			}
		}
	char _dta[Res_Xi] "`xi_names'"
...
program define Xij /* <names-maybe-with-@>[, atwl(string) */
	...
	while "`1'" != "" & "`1'"!="," {
		local list "`list' `1'"
		/* can keep labels only if going long to wide */
		capture local theLab : var lab `1'
		if !_rc {
			local lablist "`lablist' `"`theLab'"'"
			}
		mac shift
...
/* 	local dsnew "rs_dsnew" */
/* 	local dsvars "rs_dsvars" */
/* 	local hold "rs_hold" */
	if "$Res_Xi" != "" {
		tempfile dscons
		* local dscons "rs_dscons"
	}
	quietly {
...
/*
	...
	go through $ReS_Xij and rename and relabel each ${ReS_Xij}#
*/
...
program define Widefix /* # */ /* reshape wide utility */
	local val "`1'"
	parse "$ReS_Xij", parse(" ")
	/* since synchronized lists are in use, get rid of mac shift stuff */
	local cnt 1
	while "``cnt''" != "" {
		...
		Subname ``cnt'' $ReS_atwl
		rename $S_1 `new'
		local theLab : word `cnt' of $ReS_Xij_lab
		if "`theLab'"!="" {
			label var `new' "`theLab' - `val'"
			}
		local cnt = `cnt' + 1
	}
end
...
program define Longdo /* reshape long dolist utility */
	...
	local cnt 1
	foreach var of global Res_Xi {
		local theLab : word `cnt' of $ReS_Xi_lab
		label var `var' "`theLab'"
		local cnt = `cnt' + 1
	}
	parse "$ReS_Xij", parse(" ")
	/* once again, synchronized processing... */
	local cnt 1
	while "``cnt''" != "" {
		Subname ``cnt'' `val'
		local van "$S_1"
...
program define ConsLab
	local cnt 1
	foreach var of global Res_Xi {
		local theLab : var lab `var'
		local lablist "`lablist' `"`theLab'"'"
	}
	global ReS_Xi_lab `"`lablist'"'
	char _dta[ReS_Xi_lab] $ReS_Xi_lab
end
exit
