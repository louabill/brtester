*! version 4.2.1  March 30, 2004 @ 11:06:20
program define reshape8
	/* 4.2.1 - fixed a couple of really bad, showstopping bugs which somehow went undetected earlier */
	...
	/* for debugging */
	global ReS_Keep "ReS_ver ReS_i ReS_j ReS_jv ReS_jv2 ReS_Xij ReS_Xi ReS_atwl ReS_str ReS_Xij_long ReS_j_lab ReS_Xij_lab ReS_Xi_lab"
	/* for real */
		...
		foreach val of global ReS_jv {
			...
			foreach xijvar of global ReS_Xij {
				...
				if _rc {
					...
					local ovlist `ovlist' `oldvar'
					local nvlist `nvlist' `newvar'
					local ifmiss "`ifmiss' & missing(`oldvar')" 
					}
				}
			keep $ReS_i `ovlist'
			drop if `ifmiss'
			local vnum 1
			foreach var of local ovlist {
				local newvar : word `vnum' of `nvlist'
				rename `var' `newvar'
				local vnum = `vnum' + 1
				}
			gen `type' $ReS_j = `val'
			tempfile tmp`val'
			local filelist `filelist' `tmp`val''
			save `tmp`val''
			}
...
program define FillvalW
	foreach xvar of global ReS_Xij {
		...
		foreach var of local varlist {
			...
			local valuelist : list valuelist | value
			}
		}
	if "`valuelist'"=="" {
